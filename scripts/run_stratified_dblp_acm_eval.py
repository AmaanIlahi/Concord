...
from load_benchmark import load_dblp_acm_stratified_sample
from app.db import SessionLocal
from app.services.bulk_embedding import embed_dataset_records

BASE_URL = "http://127.0.0.1:8000"
REQUEST_TIMEOUT = 3600
DATASET_PAIR_NAME = "dblp_acm_stratified_150"
...
def main():
    print(
        f"Building stratified DBLP-ACM sample "
        f"({POSITIVE_COUNT} positives + {NEGATIVE_COUNT} negatives)..."
    )
    dblp_path, acm_path, gold_path = load_dblp_acm_stratified_sample(
        positive_count=POSITIVE_COUNT, negative_count=NEGATIVE_COUNT
    )
    print(f"  gold file scoped to sample: {gold_path.name}")

    print(f"\nUploading {dblp_path.name} ...")
    (dblp_id, dblp_count), upload_a_time = timed(
        "Upload DBLP (stratified sample)", lambda: upload(dblp_path)
    )
    print(f"  dataset_id={dblp_id}  row_count={dblp_count}")

    print(f"\nUploading {acm_path.name} ...")
    (acm_id, acm_count), upload_b_time = timed(
        "Upload ACM (stratified sample)", lambda: upload(acm_path)
    )
    print(f"  dataset_id={acm_id}  row_count={acm_count}")

    def do_bulk_embed():
        db = SessionLocal()
        try:
            result_a = embed_dataset_records(db, dblp_id)
            result_b = embed_dataset_records(db, acm_id)
    ...
    def do_match():
        return requests.post(
            f"{BASE_URL}/match",
            json={"dataset_a_id": dblp_id, "dataset_b_id": acm_id},
            timeout=REQUEST_TIMEOUT,
        )
    ...
    print("=" * 70)
    print(f"DBLP records uploaded:      {dblp_count}")
    print(f"ACM records uploaded:       {acm_count}")
    ...
    print("\nStratified DBLP-ACM pipeline + eval completed without errors.")
